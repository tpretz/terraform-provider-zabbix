...
- `active` (Boolean) Use an active agent check: the agent connects to the server and asks for its checks, rather than the server polling the agent. Active checks need no inbound connectivity to the host
