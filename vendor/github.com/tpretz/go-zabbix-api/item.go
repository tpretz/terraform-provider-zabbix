...
type Item struct {
	...
	DataType     DataType  `json:"-"` // removed in Zabbix 5.4, never send
	Delta        DeltaType `json:"-"` // removed in Zabbix 5.0, never send
	...
	// ItemParent is populated by selectHosts on read; it is read-only and
	// must never be sent on create/update (Zabbix rejects it as unexpected).
	...
	// note: the json option must be lowercase "omitempty" to be honoured by
	// encoding/json, otherwise this read-only field is always serialized
