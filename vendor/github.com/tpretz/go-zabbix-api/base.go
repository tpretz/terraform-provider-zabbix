...
type API struct {
	Auth      string      // session id, filled by Login()
	ApiToken  string      // API token, used instead of Login() when set
...
type Config struct {
	...
	// ApiToken authenticates with a Zabbix API token instead of a
	// username/password session. Required when the account is protected by
	// MFA, since user.login cannot complete an MFA challenge.
	ApiToken string
}

// zabbixAuthHeaderMinVersion is the first Zabbix version accepting credentials
// in the HTTP Authorization header. Zabbix 7.2 removes the "auth" request
// field entirely, so the header is the forward-compatible transport.
const zabbixAuthHeaderMinVersion = 60000

// methodNeedsNoAuth reports whether a method must be called with no
// credentials at all. Zabbix rejects apiinfo.version outright if either the
// auth field or the Authorization header is present.
func methodNeedsNoAuth(method string) bool {
	return strings.EqualFold(method, "apiinfo.version")
}
...
func NewAPI(c Config) (api *API, err error) {
	...
	api.Config.VersionString = rawVersion
...
func (api *API) callBytes(method string, params interface{}) (b []byte, err error) {
	// Pick the credential to present: an explicit API token wins over a
	// session id obtained from Login().
	credential := api.ApiToken
	if credential == "" {
		credential = api.Auth
	}

	// Zabbix >= 6.0 accepts credentials in the Authorization header, which is
	// the only supported transport from 7.2 onwards. Older versions only read
	// the "auth" request field.
	useHeader := api.Config.Version >= zabbixAuthHeaderMinVersion
	// An API token is only ever valid in the header, never in the auth field.
	if api.ApiToken != "" {
		useHeader = true
	}

	if methodNeedsNoAuth(method) {
		credential = ""
	}

	authField := ""
	if credential != "" && !useHeader {
		authField = credential
	}

	id := atomic.AddInt32(&api.id, 1)
	jsonobj := request{"2.0", method, params, authField, id}
...
// SetApiToken configures token based authentication. Unlike Login() this makes
// no API call; use CheckAuthentication() to verify the token eagerly.
func (api *API) SetApiToken(token string) {
	api.ApiToken = token
	api.Config.ApiToken = token
}

// CheckAuthentication verifies the currently configured credential by issuing a
// cheap authenticated request. Returns an error if the credential is rejected.
//
// API tokens cannot be validated with user.checkAuthentication (that method is
// session oriented), so a minimal read is used instead.
func (api *API) CheckAuthentication() (err error) {
	_, err = api.CallWithError("usergroup.get", Params{
		"countOutput": true,
		"limit":       1,
	})
	return
